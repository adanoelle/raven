...
Direction is locked at activation (movement input if moving, else aim direction).
...
The melee hit check uses `point_in_cone()` from `src/ecs/systems/hitbox_math.hpp`:
...
2. **Angle check**: `dot(normalize(aim), normalize(dir_to_target)) >= cos(half_angle)`
...
```
update_input          (skip velocity during Dash)
update_melee       <- arc check, disarm, spawn pickups
update_dash        <- burst velocity, invulnerability
update_shooting
update_emitters
update_ai             (disarmed -> Chaser override)
animation state logic (extended with Melee/Dash states)
update_animation
update_movement
