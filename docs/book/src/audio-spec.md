...
Reference document for sound effects, music, technical requirements, file formats,
...
presented here so the decision can be made later with full context. All three can
work with the current SDL_mixer pipeline (see [ADR-0010](decisions/0010-sdl2-mixer-audio.md)).
...
recommended characteristics to guide creation regardless of which sonic direction
is chosen.
...
loop point should be at the end of the file — SDL_mixer's `Mix_PlayMusic(-1)` loops
from the beginning. For OGG files, use the `LOOPSTART` and `LOOPLENGTH` tags if
a non-zero loop point is needed.
...
`Mix_VolumeMusic(sdl_volume)` for music, `Mix_VolumeChunk(chunk, sdl_volume)` for
individual SFX, or `Mix_Volume(channel, sdl_volume)` for a specific channel.
...
Numbered variants (`_01`, `_02`) are for randomized playback to reduce repetition.
...
- [ ] Filename follows naming convention (`sfx_` or `mus_` prefix, lowercase, underscores)
- [ ] File is in the correct directory (`assets/audio/sfx/` or `assets/audio/music/`)
