...
One Aseprite template file per tier, reused for every character, enemy, or
boss at that size.

> **The templates already exist** in `art/templates/` (all four tiers),
> generated by `tools/gen_templates.lua`. Because Aseprite's guide lines
> aren't scriptable, the generated files carry the same information as a
> locked, semi-transparent **"guides" pixel layer** instead: a cyan body-zone
> box, orange proportion lines, a red feet-lock line, and grey center lines.
> Draw on the `art` layer above it, and hide the guides layer before export.
> Real View > Guides can be added on top by hand (steps below) — they and
> the pixel layer serve the same purpose, so use whichever reads better.

The steps below describe building a template from scratch, and are the
reference for what the guide positions mean.
