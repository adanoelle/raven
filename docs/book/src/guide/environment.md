...
Raven uses a [Nix flake](https://nixos.wiki/wiki/Flakes) to define a fully
reproducible development environment. Every tool, library, and compiler version
is pinned — no "works on my machine" problems.
...
These are pulled automatically at configure time by
